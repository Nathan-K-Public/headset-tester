...
struct ringbuffer_t {
  ...
  */
  ...
  // overwrite version, affects head, reads tail, element overwritten if overflow
  void push_back_o(const T element) {
    register uint16x2_t temp = { offsets.both };

    elements[temp.head++] = element;
    temp.head &= CAPACITY;
    if ( (temp.head == temp.tail) ) { // full
      temp.tail++;
      offsets.tail = temp.tail & CAPACITY;
    }

    offsets.head = temp.head & CAPACITY;
    return;
  }
  ...
  //POP OFF THE HEAD (back of queue)
  // affects tail, reads head
  POP_T pop_back(void) {
    register uint16x2_t temp = { offsets.both };

    if ( (temp.head - temp.tail) & CAPACITY ) { // !empty
//      Serial.println(temp.head);
//      Serial.println(elements[temp.head]);
      POP_T elem = elements[--temp.head];
      offsets.head = temp.head & CAPACITY;
      return elem;
    }
  ...
  //POP OFF THE TAIL (front of queue)
  // affects tail, reads head
